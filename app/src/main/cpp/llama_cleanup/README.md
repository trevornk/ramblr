...
| `LLMInference.cpp` | Same commit, **plus targeted #75/#87/#74/#92 fixes** over the original: EOG path no longer double-strdups (leaked one full response copy per completion) and honors `_storeChats`; `startCompletion` deletes the previous `_batch` before allocating; `minP` now actually adds a min-p sampler instead of being logged and ignored; destructor frees an owned chat template; `storeChats=false` is now genuinely one-shot on a reused instance (`startCompletion` clears the KV cache, `stopCompletion` clears `_messages` and the partial-UTF-8 accumulator), which `LocalCleanupModelHolder`'s instance caching depends on; and a ggml abort callback + a `setInferenceBudgetMs` deadline give a single otherwise-uninterruptible `llama_decode` a mid-decode escape hatch so a pathologically slow decode can't outrun the waterfall's wall-clock budget (#92). Divergences are flagged in the file's header comment. |
| `llama_cleanup_jni.cpp` | **Adapted** from `smollm.cpp` (same commit): JNI symbol names renamed to this app's package (`Java_com_trevornk_ramblr_LlamaCppInference_*`), and trimmed from 8 exported functions to the 6 `LlamaCppInference.kt` actually declares (dropped `getResponseGenerationSpeed`/`getContextSizeUsed`/`benchModel` -- perf/bench accessors unused by a single-shot cleanup call). See the file's own header comment for the full adaptation diff summary. |
