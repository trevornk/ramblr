 ...
 * .credentialSlot] returns null) and never fails at the "host unreachable" level, so grouping it
 * with other steps is harmless but pointless -- it either succeeds or fails as a single step
 * regardless of neighbors.
 */
enum class CleanupStepGroup { OMNIROUTE, OPENAI_DIRECT, ANTHROPIC_DIRECT, GEMINI_DIRECT, LOCAL_LLM }
 ...
 * free at the margin despite being a network fallback -- and LOCAL_LLM has no incremental cost
 * either, so only the two direct-provider groups count as a "paid fallback" from the user's cost
 * perspective.
 */
