...
class AutomationOffReceiver : BroadcastReceiver() {
    ...
    companion object {
        ...
        /** #254 investigation: read-only status snapshot, see [handleDiagnostic]'s kdoc. Reply
         *  comes back as the ordered broadcast's result DATA string (`adb shell am broadcast`
         *  prints it as `data="..."`), not a separate extra, so no extras-parsing dependency is
         *  needed to read it from a plain shell/automation-tool broadcast call. */
